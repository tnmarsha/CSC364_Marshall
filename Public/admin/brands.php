<?php 
	...
	$brand = sanitize ($_POST['brand']);
		...
		if($_POST['brand'] == ''){
			...
			if($count > 0){
			$errors[] .= $brand.'That brand already exists please choose another name';
				}
			//display errors
			if(!empty($errors)){
...
?>
<h2 class="text-center">Systems</h2><hr>
<!-- System Form -->
<div class="text-center">
	<form class="form-inline" action="brands.php<?=((isset($_GET['edit']))?'?edit='.$edit_id:'');?> " method="post">
		...
		if(isset($_GET['edit'])){
			...
			<label for="brand"><?=((isset($_GET['edit']))?'Edit':'Add A');?> Item:</label>
			<input type="text" name="brand" id="brand" class="form-control" value= "<?=$brand_value;?>">
			<?php if (isset($_GET['edit'])): ?>
			<a href="brands.php" class="btn btn-default">Cancel</a>
			<?php endif; ?>
			<input type="submit" name="add_submit" value="<?=((isset($_GET['edit']))?'Edit':'Add');?> System" class="btn btn-success">
			</div>
		</form>
	</div><hr>
